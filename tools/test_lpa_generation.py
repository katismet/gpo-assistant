#!/usr/bin/env python3
"""
Тест генерации ЛПА PDF из DOCX шаблона
"""
...
def test_lpa_generation():
    ...
    print("🧪 Тестируем генерацию ЛПА PDF...")
    ...
    try:
        ...
        if not template_path.exists():
            ...
            print("📝 Создайте файл lpa_template.docx в директории app/templates/pdf/")
            return False
        
        print(f"✅ Шаблон найден: {template_path}")
        
        # Генерируем PDF
        pdf_path = pdf_service.generate_lpa_pdf(test_data)
        
        if os.path.exists(pdf_path):
            file_size = os.path.getsize(pdf_path)
            print(f"✅ ЛПА PDF успешно создан!")
            print(f"📄 Файл: {pdf_path}")
            print(f"📏 Размер: {file_size:,} байт")
            return True
        else:
            print(f"❌ PDF файл не создан: {pdf_path}")
            return False
...
if __name__ == "__main__":
    print("🚀 Запуск теста генерации ЛПА PDF")
    print("=" * 50)
